...
When MCP implicitly selects a repository-local graph, `search` also reads the globally configured data directory and merges both result sets. `task_list` includes global tasks and tasks assigned to the current project from that graph. Global results carry `global:<id>` references; `show`, node edits, links, and task mutations route those references back to the global graph. Cross-graph links are rejected. New `add`, `learn`, and `task_add` calls can set `graph="global"` or pass `source_refs` containing a `global:<id>`; mixed evidence routes new work to the global graph. Explicit profile selection remains isolated.
