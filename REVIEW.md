...
- **Forward-only migrations**: schema changes in `schema.py` append migrations; never edit or reorder applied ones. Existing databases must open cleanly after the change. One exception: a definition-only repair (a trigger or index that holds no data, whose old and new definitions both work with every supported build) may be applied in place when a store opens, without a version bump, because a versioned migration takes a full-database snapshot for a change that touches no data. Such a repair must be idempotent, must skip a busy or read-only store, and must be named in the CHANGELOG.
